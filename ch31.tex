...
\author{ % alphabetical order please
Kurt Konolige\\
Google Inc., U.S.A.\\
\texttt{konolige@google.com}\\
\and
Andreas N{\"u}chter\\
Robotics and Telematics\\
University of W{\"u}rzburg, Germany \\
\texttt{andreas@nuechti.de} \\
\\
\\
\\
}
...
\index{Range data} Range data is a $2\frac{1}{2}$ or 3D representation
...
scene.  Often only a single \index{range image} range image is used at
...
shows a \index{registered reflectance image} registered reflectance
...
%%{\bf Precision, repeatability, accuracy}

\subsection{Triangulation}

Triangulation sensors measure depth by determining the angle formed by
the rays from a world point to two ``sensors.''
The sensors are separated by a baseline of length
...
image plane from the primary ray; this offset $x$ is
...
(not to be confused with structured light), using different
kinds of support neighborhoods, and regularization methods.
...
small group of pixels.  The PrimeSense technology, used in the
Kinect, projects a known pattern of dots, and decodes a small
...
\index{Time of flight range sensors}
...
%{\bf TODO: minimum range, ambiguity interval, accuracy based on
%  multiple readings, beam size, scanning rates, multiple reflections,
%  obscurants}
...
available. Velodyne \cite{velodynelidar} makes three devices, with
16, 32 and 64 beams arranged vertically, that acquire point data at
rates of up to 15 scans/s (1.3 MPixel/s), with a full 360 degree
horizontal scan and a 27 degree vertical FOV from the laser array.
The laser pulses are 5 ns long, and depth precision is approximately 2
cm.  These devices are typically used in autonomous driving, for
environment reconstruction and obstacle avoidance.
...
timing electronics; a typical device from ASC \cite{stettner} has 128$\times$128
...
subsection.  The table below lists the characteristics of one of ASC's
devices.
...
\index{Modulation-based range sensors}
...
phase difference and the intensity:
 %
{\epsfxsize = 0.47\textwidth \epsfbox{BOOKFIGS/phase-filter.eps}}
 %
...
pixel, in a compact sensor \cite{canesta04,swissranger03}.  Because
...
\cite{wong11}.  The electronics required for mixing and frequency
...
technology is in reference \cite{wong11}.  The two main
...
\caption{Range device characteristics (from \cite{wong11}).
...
\begin{table}
\begin{tabular}{|p{1.2in}p{1.9in}|l|}
\hline
Correlation surface  & \cite{matthies93}\\
\hline
\verb+  + peak width & wide peak indicates poor \\
& feature localization \\
\verb+  + peak height & small peak indicates poor \\
& match \\
...
\eqref{eq31.disparity-range} with respect to disparity:
\begin{equation}
...
consumer electronics device in history \cite{fastsell}.  The Kinect
uses a structured-light stereo technique from PrimeSense (Bought out
by Apple Inc.) to give a reasonable balance of spatial and depth
precision, in a highly-integrated device, at a very low price.
...
Figure \ref{fig:kinect pattern}).  The pattern consists of bright dots on
a grid, with unique combinations of dots over a horizontal area.  A
camera with resolution 1280$\times$960 captures the IR image at a horizontal
offset of 7.5 cm from the projector, and correlates a 19$\times$19 block
against the known dot pattern for a given horizontal line.  If a match
is found, the position of the pattern in the known projected pattern
gives the angle subtended by the reflecting object to the projector
and camera, and hence the distance.  A depth image is returned at a
resolution of 640$\times$480, with 11 bits of depth precision, over a FOV of
approximately 50 degrees; the depth is interpolated to a precision of
1/8 pixel. Error characteristics for the Kinect have been studied in
\cite{Khoshelham}. The depth decision is limited by the resolution, similarly to \eqref{eq31.disparity-resolution}.

\begin{figure}  
{\epsfxsize = 0.49\textwidth \epsfbox{BOOKFIGS/kinect_pattern.eps}}
\caption{The laser grid of the Kinect for calculating depth.
\label{fig:kinect pattern}}
\end{figure}

%Error characteristics for the Kinect have been studied in
%\cite{Khoshelham}. Based on Equation
...
\item {\bf 3D Point Features \index{Point Features}}:
  ...
  Fig.~\ref{fig:scansift} shows SIFT features~{lowe} extracted from a
  3D scanner with calibrated reflectivity values.
  \begin{figure}
    \centering
...
\item {\bf Planes}:
  ...
  \begin{figure}
    ...
    \caption{Transformation of three points from $\mathbbm{R}$ to
...
\begin{figure}
    \includegraphics[width=0.99\linewidth]{BOOKFIGS/mc_cases_corrected}
...
discussed in chapter 37 solve a very similar problem. In addition to
registering multiple views, they estimate a map.  Multiple-View
...
\begin{enumerate}
  \item
    identification of grasping points (see Chapters 27 and 28),
  \item
    identification of a collision free grasp (see Chapters 27 and 28),
  \item
    recognition of parts to be manipulated (see Chapter 23) and
  \item position estimation of parts for manipulation (see Chapters 23 and 42).
\end{enumerate}
%
...
\item
  identification of a navigable groundplane (see Section \ref{ch31.3}),
\item
  identification of a collision free path (see Chapter 35),
\item
  identification of landmarks (see Chapter 36) and
\item
  estimation of vehicle location (see Chapter 40).
\end{enumerate}
...
\verb+http://archive.darpa.mil/grandchallenge05/gcorg/index.html+, accessed April 24, 2015.
...
Focus Robotics Inc.,
\verb+www.focusrobotics.com+, accessed April 24, 2015.
...
\bibitem{canesta04}
S.B.~Gokturk, H.~Yalcin and C.~Bamji,
...
\bibitem{zcam}
G. Iddan, G. Yahav ``3D imaging in the studio (and elsewhere...)''.
Proceedings of SPIE 4298, San Jose, April 2003.
...
\verb+www.kartorobotics.com+, accessed April 24, 2015.
...
\bibitem{Khoshelham}
  K. Khoshelham, S. O. Elberink, ``Accuracy and Resolution of Kinect Depth Data for Indoor Mapping Applications'', in \em{Sensors}:12(5), 2012.
...
\bibitem{farophoton10}
D.~D.~Lichti,
``A Review of Geometric Models and Self-Calibration Methods for Terrestrial Laser Scanner'',
Bol. Ciȇnc. Geod., pp  3-19, Curitiba, 2010.
...
\bibitem{swissranger03}
T. Oggier, M. Lehmann, R. Kaufmannn, M. Schweizer, M. Richter, P. Metzler, G. Lang,
F. Lustenberger and N. Blanc,
``An all-solid-state optical range camera for 3D-real-time imaging with sub-centimeter depth-resolution (SwissRanger)'',
Proc.~SPIE Vol. 5249, pp. 634-545, 2003.
...
\verb+https://www.ri.cmu.edu/research_project_+
\verb+detail.html?project_id=595+, accessed April 24, 2015.

\bibitem{pointclouds.org}
Point Cloud Library (PCL),
\verb+pointclouds.org+, accessed April 24, 2015.

\bibitem{ptgrey}
Point Grey Research Inc.,
\verb+www.ptgrey.com+, accessed April 24, 2015.
...
\verb+www.cs.stanford.edu/group/stair+, accessed April 24, 2015.
...
\verb+http://www.advancedscientificconcepts.com/+\\
\verb+technology/documents/ThreeDimensionalFlash+\\
\verb+LadarFocalPlanes-ISSSRPaper.pdf+,
...
\bibitem{tridicam}
  TriDiCam GmbH,
  \verb+http://www.tridicam.de/+
  \verb+en.html+, accessed April 24, 2015.
...
\bibitem{tyzx}
TYZX Inc.,
\verb+www.tyzx.com+, accessed April 24, 2015.

\bibitem{velodynelidar}
  Volodyne -- High definition Lidar.
  \verb+velodynelidar.com/+, accessed April 24, 2015.
...
\bibitem{fastsell}
M.~W. Walker, L.~Shao, and R.~A. Volz,
\verb+http://en.wikipedia.org/wiki/List_of_+\\
\verb+fastest-selling_products+,
accessed April 24, 2015.


\bibitem{wong11}
U.~Wong, A.~Morris, C.~Lea, J.~Lee, C.~Whittaker, B.~Garney and R.~Whittaker, Red,
``Comparative evaluation of range sensing technologies for underground void modeling'',
Proc.~IROS, pp 3816-3823, 2011.
