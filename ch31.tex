...
If $n$-views have to be registered, any sequential application of ICP
will accumulate errors, and therefore the ICP algorithm
(cf. section~\ref{sec:icp6D}) has to be extended. The global error
function becomes  
\begin{align}
...
pairs for all overlapping views have been found,
...
\FOR {$i = 0$ to \textit{maxIterations}}
  ...
  \STATE {Calculate $n$ transformations ($\M R, \V t$) that minimizes the
    error function Eq. (\ref{DMinGlobal})}
...
the probabilistic approach first proposed in \cite{Lu1997a} to
6\,DoF. For a more detailed description of the extension refer to
\cite{RAS2007}

For each pose ${\V X}$, the term
$\bar{\V X}$ denotes a pose estimate, and $\Delta {\V X}$ is the pose error.

The positional error of two poses ${\V X}_j$ and
${\V X}_k$ is described by:
\begin{align*}
E_{j,k} &= \sum^m_{i=1} \norm{ {\V X}_j \oplus {\V d}_i - {\V X}_k \oplus
{\V m}_i }^2 = \sum^m_{i=1} \norm{ {\V Z}_i ({\V X_j}, \V X_k) }^2.
\end{align*}
...
$E_{j,k}$ can be linearized by use of a Taylor expansion:
...
\subsection{Model Matching and Multiple-View Registration\label{match31}} \index{Multiple-View Registration}
