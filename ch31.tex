...
Although, implementations of ICP (cf. algorithm~\ref{algo:ICP})
use a maximal distance for closest points to handle partially
overlapping point sets. In this case, the proof about ICP's monotonic
cnvergance in~\cite{Besl_1992} no longer holds, since the number of
points as well as the value of $E(\M R, \V t)$ might increase after
...
\item {\bf 3D Point Features \index{Point Features}}:
  ...
  texture, e.g., co-registered color images or scan reflectivity, are
  widely used for
  registration~\cite{Boehm_2007,Weinmann_2011_RegistrationOfTLS,FIXME}.
  Fig.~\ref{fig:scansift} shows SIFT features extracted for a 3D
